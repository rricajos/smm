<script lang="ts">
  ...
  let {
    ...
    onacceptallrules,
    onaccepttemplate,
    onacceptalltemplates,
    onacceptconsolidation,
    onacceptallconsolidations,
  }: Props = $props();
...
<div class="conversation-bar">
  <div class="conversation-current">
    ...
    <button
      ...
      title={$t('ai_new_conversation')}>+</button
    >
  </div>
      ...
      {#each conversations as conv}
        <div class="conv-item" class:active={conv.id === currentConversation.id}>
          <button class="conv-item-btn" onclick={() => handleSwitchConversation(conv.id)}>
            ...
            <span class="conv-item-count"
              >{$t('ai_conv_msg_count', { count: conv.displayMessages.length })}</span
            >
          </button>
          {#if conversations.length > 1}
            <button
              ...
              title={$t('ai_delete_conversation_title')}>&times;</button
            >
          {/if}
        </div>
      {/each}
    ...
    {:else}
          ...
          <div class="chat-bubble {msg.role}">
            ...
            {#if msg.ruleConsolidationProposals && msg.ruleConsolidationProposals.length > 0}
              <ProposalBlock
                ...
                onacceptall={() =>
                  onacceptallconsolidations(msgIdx, msg.ruleConsolidationProposals || [])}
              />
            {/if}
          </div>
        {/each}
      ...
      {#if selectedFolder}
        <div class="folder-context-bar">
          ...
          <button
            class="folder-context-clear"
            onclick={() => onclearfolder?.()}
            title={$t('ai_folder_context_clear')}>&times;</button
          >
        </div>
      {/if}
      ...
      <div class="chat-input-area">
        ...
        ></textarea>
        <Button variant="primary" onclick={handleSend} disabled={chatLoading || !chatInput.trim()}
          >{$t('common_send')}</Button
        >
      </div>
    {/if}
  </div>
...
<style>
  ...
  .conv-item.active {
    background: var(--bg-hover, #e8e8ee);
  }
  .conv-item-btn {
    flex: 1;
    display: flex;
  ...
  .chat-bubble.user .markdown-content :global(code) {
    background: rgba(255, 255, 255, 0.2);
  }
  .chat-bubble.user .markdown-content :global(pre) {
